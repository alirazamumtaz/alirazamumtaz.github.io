...
/**
 * Blog (/blog) — blog.md. The complete, filterable archive: quiet search
...
export default function Blog() {
  ...
  useEffect(() => {
    ...
    // Warm the full-text search index alongside the archive (6 small posts).
    searchRecords()
      .then((recs) => {
  ...
  // stagger 35ms/item (180ms, y:8) per blog.md §2.
  const initialKeyRef = useRef<string | null>(null)
  if (filtered && initialKeyRef.current === null) initialKeyRef.current = listKey
  const isInitialList = listKey === initialKeyRef.current
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 sm:px-6">
      ...
      <header className="pb-8 pt-10 sm:pt-14">
        ...
        >
          Long-form writeups on vulnerability research, exploit development, and reverse
          engineering: root-cause analysis, PoCs, and techniques.
        </motion.p>
        {metaLine && (
      ...
      >
        ...
        {tagCounts.length > 0 && (
          <div
            className="mt-3 flex items-center gap-2 overflow-x-auto [mask-image:linear-gradient(to_right,black_92%,transparent)] [scrollbar-width:none] md:flex-wrap md:overflow-visible md:[mask-image:none] [&::-webkit-scrollbar]:hidden"
            role="group"
