...
import { formatPostDate } from '@/components/PostListItem'
import { extractHeadings, loadPost, loadPosts } from '@/lib/posts'
import type { Post, PostMeta } from '@/lib/posts'
import { cn } from '@/lib/utils'

/**
 * Post (/blog/:slug) — post.md. The long-form reading experience.
 ...
 * Extras per post.md notes: 2px reading-progress bar at the header's bottom
...
/** Document title + OG/Twitter meta per post (post.md notes). */
function useDocumentMeta(post: Post | null) {
  useEffect(() => {
    if (!post) return
    const title = post.title.trim()
    const url = `${SITE_URL}/blog/${post.slug}`
    const image = post.ogImage ?? `${SITE_URL}/og-cover.png`

    document.title = `${title} · locus-x64`
    ...
    return () => {
      const siteDesc =
        'Vulnerability research, exploit development, and reverse engineering notes by Ali Raza.'
      document.title = 'locus-x64 | Security Researcher'
      setMeta('name', 'description', siteDesc)
      setMeta('property', 'og:type', 'website')
      setMeta('property', 'og:title', 'locus-x64 | Security Researcher')
      setMeta('property', 'og:description', siteDesc)
      setMeta('property', 'og:url', `${SITE_URL}/`)
      setMeta('property', 'og:image', `${SITE_URL}/og-cover.png`)
      setMeta('property', 'article:published_time', '')
      setMeta('name', 'twitter:title', 'locus-x64 | Security Researcher')
      setMeta('name', 'twitter:description', siteDesc)
      setMeta('name', 'twitter:image', `${SITE_URL}/og-cover.png`)
    }
  }, [post])
}

/* ---------------- reading progress (design.md §5) ---------------- */
...
/* ---------------- print stylesheet (post.md notes) ---------------- */
...
export default function PostPage() {
  ...
  const showSidebar = headings.length >= 3
  ...
  const editUrl = `https://github.com/locus-x64/locus-x64.github.io/edit/main/public/posts/${post.slug}.md`
