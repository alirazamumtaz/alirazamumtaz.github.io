...
/**
 * Navbar — design.md §6.1.
 * Sticky 64px header, backdrop blur over 82% bg, bottom hairline.
 * Wordmark `locus-x64` (Space Grotesk 700, `-` in accent) + 7px accent dot.
...
const NAV_ITEMS = [
  ...
  { to: '/contributions', label: 'Contributions', end: false },
  { to: '/about', label: 'About', end: false },
]

export const GITHUB_URL = 'https://github.com/locus-x64'
...
export function Wordmark({ className }: { className?: string }) {
  return (
    ...
    >
      locus<span className="text-accent">-</span>x64
    </span>
  )
}

/** 7px accent dot — decorative brand mark (design.md §6.1, Assets → emblem). */
  ...
  return (
    ...
    >
      <div className="mx-auto flex h-full max-w-3xl items-center justify-between px-5 sm:px-6 lg:max-w-6xl">
        <Link to="/" className="flex items-center gap-2" aria-label="locus-x64, home">
          <Wordmark />
          <AccentDot />
        </Link>
        ...
        <nav aria-label="Primary" className="hidden items-center gap-6 md:flex">
          ...
          <div className="flex items-center gap-1 pl-6">
            <a href={RSS_URL} className={iconLinkClass} aria-label="RSS feed">
              <Rss size={18} />
            </a>
            <a
              href={GITHUB_URL}
