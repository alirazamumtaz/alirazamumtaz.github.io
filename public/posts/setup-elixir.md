...
excerpt: "Explore source code in your browser - Particularly useful for the Linux kernel and other low-level projects in C/C++. A guide to setting up your own Elixir instance."
date: "2024-03-27"
readingTime: 8
tags: ["Vulnerability Research", "Code Auditing", "Tools"]
---

![Elixir Cross Referencer](images/posts/setup-elixir/setup-elixir-backdrop.jpg)
...
Now for the fun part! We can tell our running Elixir instance to index a project. We'll use the `zlog` project as an example.
...
- `/srv/elixir-data`: The path inside the container where data should be stored.
- `zlog`: The name for your project.
- `https://github.com/HardySimpson/zlog.git`: The URL of the git repository to index.
...
For a more permanent or production-like setup, using `docker-compose` is a great idea. Create a `docker-compose.yml` file in the `elixir` directory:
